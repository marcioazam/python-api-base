...
Usage:
    from my_api.shared.multitenancy import TenantRepository, TenantContext

    # Repository with automatic tenant filtering
    repo = TenantRepository[Item, ItemCreate, ItemUpdate](
        session=session,
        model_class=Item,
    ...
    items = await repo.get_all()  # Only returns tenant_123's items
"""
...
TenantId = TypeVar("TenantId", str, int)
