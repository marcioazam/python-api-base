...
class GenericCRUDRouter[T, CreateDTO, UpdateDTO, ResponseDTO]:
    ...
    def _setup_create_route(
        self, response_model: type, create_model: type, use_case_dep: Callable
    ) -> None:
        @self.router.post(
            "",
            response_model=ApiResponse[response_model],
    ...
    def _setup_update_route(
        self, response_model: type, update_model: type, use_case_dep: Callable
    ) -> None:
        @self.router.put(
            "/{id}", response_model=ApiResponse[response_model], summary="Update item"
        )
    ...
    def _setup_delete_route(self, use_case_dep: Callable) -> None:
        @self.router.delete(
            "/{id}", status_code=status.HTTP_204_NO_CONTENT, summary="Delete item"
        )
    ...
    def _setup_bulk_routes(
        ...
        @self.router.delete(
            "/bulk", response_model=BulkDeleteResponse, summary="Bulk delete items"
        )
