...
class SecuritySettings(BaseSettings):
    ...
    secret_key: SecretStr = Field(
        ..., description="Secret key for signing tokens", min_length=32
    )
    cors_origins: list[str] = Field(default=["*"], description="Allowed CORS origins")
    rate_limit: str = Field(
        default="100/minute", description="Rate limit configuration"
    )
...
class RedisSettings(BaseSettings):
    ...
    token_ttl: int = Field(
        default=604800, ge=60, description="Default token TTL in seconds"
    )
...
class ObservabilitySettings(BaseSettings):
    ...
    log_format: str = Field(
        default="json", description="Log output format", pattern="^(json|console)$"
    )
